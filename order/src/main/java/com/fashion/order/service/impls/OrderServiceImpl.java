...
public class OrderServiceImpl implements OrderService{
    ...
    final OrderUpdateStatusErrorProvider orderUpdateStatusErrorProvider;
        ...
        try {
            ...
            Map<UUID, InnerOrderDetail_FromOrderRequest> odRequest = request.getOrderDetails().stream()
                ...
                .collect(Collectors.toMap(
                    o -> o.getProductSku().getId(),Function.identity(),(existing, replacement) -> existing
                ));
            ...
            CompletableFuture<Void> inventoryFuture = (!odRequest.isEmpty()) 
                ? AsyncUtils.fetchVoidWThread(() -> this.inventoryClient.checkQuantityAvailableInventoryByProductSkuId(odRequest.values(), request.getVersion()), virtualExecutor)
                : CompletableFuture.completedFuture(null);

            // CompletableFuture<Coupon> couponFuture = (request.getCoupon().getId() != null) 
            ...
            for (ProductResponse product : productResponse) {
                for (InnerProductSkuResponse pSku : product.getProductSkus()) {
                    ...
                    BigDecimal discountPerUnit = pSku.getPromotion() != null 
                        ? pSku.getPromotion().getDiscountFinal() 
                        : BigDecimal.ZERO;
                    BigDecimal finalPricePerUnit = originalPricePerUnit
                        .subtract(discountPerUnit)
                        .max(BigDecimal.ZERO);

                    BigDecimal totalPrice_Detail = calculateTotalPrice(
            ...
            // --- CREATE ORDER ---
            BigDecimal couponAmount = coupon.getType().equals(CouponEnum.PERCENT) 
            ...
            CompletableFuture<Void> saveODFuture = CompletableFuture.completedFuture(null);
            CompletableFuture<InnerLuaCouponResponse> deCouponFuture = AsyncUtils.fetchAsyncWThread(
                // () -> this.couponService.decreaseStock(coupon.getId()) , virtualExecutor);
                () -> this.couponService.decreaseStockAtomic(coupon.getId(),1,request.getVersion()) , virtualExecutor);
            
            if(orderDetails.size() > 0){
                saveODFuture = AsyncUtils.fetchAsyncWThread(
                    () -> this.orderDetailService.saveAllOrderDetail(orderDetails, savedOrder), virtualExecutor);
            }

            try {
                CompletableFuture.allOf(saveODFuture, deCouponFuture).join();
            } catch (CompletionException e) {
                if (e.getCause() instanceof ServiceException serviceException) {
                    throw serviceException;
                }
            ...
            .thenAccept((result) -> {
                if(result.isSuccess()){
                    log.info("ORDER-SERVICE: [createOrder] Saga completed successfully for order {}", savedOrder.getId());
                    // deduction coupon
                } else {
                    log.error("ORDER-SERVICE: [createOrder] Saga failed for order {}: {}", savedOrder.getId(), result.getErrorMessage());
                    // deduction coupon, update order status
                    this.compensateOrderAndCoupon(savedOrder, coupon.getId(), 1, request.getVersion());
                }
            })
            .exceptionally((ex) -> {
                log.error("ORDER-SERVICE: [createOrder] Saga exception for order {}", savedOrder.getId(), ex);
                this.compensateOrderAndCoupon(savedOrder, coupon.getId(), 1, request.getVersion());
                return null;
    ...
    @Override
    // @Transactional(rollbackFor = ServiceException.class)
    public OrderResponse updateOrderStatus(UUID id, OrderEnum newStatus, String note, Long version){
        return null;
        // try {
        //     UserInsideToken currentUser = SecurityUtils.getCurrentUserId().orElseThrow(
        //         () -> new ServiceException(EnumError.ORDER_USER_INVALID_ACCESS_TOKEN, "auth.accessToken.invalid")
        //     );
        //     CompletableFuture<ApiResponse<UserResponse>> userFuture = AsyncUtils.fetchAsyncWThread(
        //         () -> this.identityClient.getInternalUserById(currentUser.getId(), version), virtualExecutor);
        //     Order order = this.findRawOrderById(id);
        //     if(order == null){
        //         throw new ServiceException(
        //             EnumError.ORDER_ERR_NOT_FOUND_ID, 
        //             "order.not.found.id",
        //             Map.of("id", id)
        //         );
        //     }
        //     // KIỂM TRA QUYỀN SỞ HỮU ĐƠN HÀNG
        //     List<String> allowedRoles = List.of(adminRole, sellerRole);
        //     if (!allowedRoles.contains(user.getRole().getSlug().toUpperCase())) {
        //         throw new ServiceException(
        //             EnumError.ORDER_PERMISSION_ACCESS_DENIED, 
        //             "permission.access.deny",
        //             Map.of("role", user.getRole().getSlug())
        //         );
        //     }
        //     // ADMIN toàn quyền
        //     if (!user.getRole().getSlug().toUpperCase().equals(RoleServiceImpl.roleADMIN)) {
        //         this.validateUserOrderOwnership(user, order); 
        //     }
        ...
        //     // 1. Xử lý khi chuyển sang CONFIRMED (Tạo Order GHN)
        //     if (newStatus == OrderEnum.CONFIRMED) {
        //         this.handleOrderConfirmation(order); 
        //     }
            
        //     // 2. Xử lý khi hủy đơn hàng (Hoàn số lượng tồn kho, coupon, promotion, tổng quan giao dịch)
        //     if (newStatus == OrderEnum.CANCELLED) {
        ...
        //     throw e;
        // } catch (Exception e) {
        //     log.error("[updateOrderStatus] Error: {}", e.getMessage(), e);
    ...
    private String generateUniqueOrderCode() {
        // 1. Lấy chuỗi ngày: 20260215
        String datePart = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd"));
        
        // 2. Tạo chuỗi ngẫu nhiên 4 ký tự (chữ in hoa và số)
        // Dùng SecureRandom cho nó an toàn hơn Random thường
        ...
        // Kết quả: 20260215HDABCD
        return datePart + "HD" + sb.toString();
    }
    ...
    private void compensateOrderAndCoupon(Order order, UUID couponId, Integer amount, Long version) {
        AsyncUtils.fetchVoidWThread(() -> {
            try {
                // Cancel order
                orderRepository.updateAtomicStatus(
                    order.getId(),
                    order.getCreatedAt(),
                    OrderEnum.CANCELLED
                );
                ...
                // Restore coupon
                this.couponService.restoreStockAtomic(couponId, amount, version);

                log.info("ORDER: Compensation completed for order {}", order.getId());
            } catch (Exception e) {
